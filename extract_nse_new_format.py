"""
NSE Bhav Copy ZIP Extractor – NEW FORMAT
-----------------------------------------
Extracts CSVs from the New Format folder.
Appends new sheets to the existing Excel log workbook
(renamed to "log sheet nse.xlsx").

Save this file at:
    C:\\Users\\Bhavya Khaitan\\BK\\Stocks\\StockData\\NSE\\extract_nse_new_format.py

Run from Command Prompt:
    cd C:\\Users\\Bhavya Khaitan\\BK\\Stocks\\StockData\\NSE
    python extract_nse_new_format.py
...
IMPORTANT:
    1. Rename your existing log file first:
       "log sheet old format.xlsx"  →  "log sheet nse.xlsx"
       (or let this script create it fresh if it doesn't exist yet)
    2. This script will ADD three new sheets to that workbook:
       - "NF Execution Summary"
       - "NF Execution Log"
       - "NF Error Log"
       It will NOT touch the existing Old Format sheets.
"""
...
SOURCE_DIR = r"C:\Users\Bhavya Khaitan\BK\Stocks\StockData\NSE\New Format"
OUTPUT_DIR = r"C:\Users\Bhavya Khaitan\BK\Stocks\StockData\NSE\New Format\Extracted"
LOG_FILE   = r"C:\Users\Bhavya Khaitan\BK\Stocks\StockData\NSE\log sheet nse.xlsx"
...
CLR_SUMMARY_HDR  = "375623"   # dark green  – new format summary (different from old)
CLR_WHITE        = "FFFFFF"
CLR_LIGHT_GREEN  = "E2EFDA"
...
def add_sheets_to_workbook(log_path, run_start, run_end,
                            total, success, skipped,
                            error_rows, execution_rows):
    """
    Open the existing workbook (or create a new one) and
    append three New Format sheets.
    """
    ...
    # ── Sheet : NF Execution Summary ─────────────────────────────────────────
    # Remove sheet if it already exists (re-run protection)
    for sheet_name in ["NF Execution Summary", "NF Execution Log", "NF Error Log"]:
        if sheet_name in wb.sheetnames:
            del wb[sheet_name]

    ws_sum = wb.create_sheet("NF Execution Summary")
    ...
    t = ws_sum["A1"]
    t.value     = "NSE Bhav Copy – New Format Extraction – Run Summary"
    ...
    for r, (label, value) in enumerate(rows, 2):
        ...
        bg = CLR_LIGHT_GREEN if r % 2 == 0 else CLR_WHITE
        for c in (lc, vc):
    ...
    # ── Sheet : NF Execution Log ──────────────────────────────────────────────
    ws_exec = wb.create_sheet("NF Execution Log")
    ...
    # ── Sheet : NF Error Log ──────────────────────────────────────────────────
    ws_err = wb.create_sheet("NF Error Log")
